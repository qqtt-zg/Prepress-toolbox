...
; 版本更新说明
AppComments=V2.4.8更新内容：重构"始终输出平张布局数"为"同时输出排版模式布局数"；1. **列重命名**：`布局数`改为`平张`列，`平张布局数`改为`卷装`列；2. **双列显示**：启用后同时计算并显示平张和卷装两种布局数；3. **高亮显示**：根据用户选择的排版模式，在对应列高亮显示；4. **重命名输出**：按用户选择的模式输出行列数；V2.4.7更新内容：优化导出路径预设功能；1. **UI宽度调整**：导出路径设置表格宽度从700拓宽至950，控制项宽度相应调整；2. **子文件夹预设**：勾选"包含子文件夹"时自动加载子文件夹，支持为每个子文件夹单独配置预设参数；3. **预设继承**：子文件夹自动继承父路径预设配置，可单独修改；V2.3.8更新内容：优化PDF操作界面底部进度反馈，实现进度条叠加显示
...
OutputBaseFilename=大诚重命名工具_v2.4.8_安装包
OutputDir=.\安装包
