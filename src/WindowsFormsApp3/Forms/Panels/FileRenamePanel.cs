...

{
    ...
    {
        private MaterialSelectFormModern _activeMaterialDialog;
        ...
        public void SetSelectedRegexPattern(string patternName)
        {
            if (string.IsNullOrWhiteSpace(patternName) || _cmbRegex == null) return;
            for (int i = 0; i < _cmbRegex.Items.Count; i++)
            {
                if (string.Equals(_cmbRegex.Items[i]?.ToString(), patternName, StringComparison.OrdinalIgnoreCase))
                {
                    _cmbRegex.SelectedIndex = i;
                    AppSettings.LastSelectedRegex = patternName;
                    AppSettings.Save();
                    return;
                }
            }
        ...
        /// </summary>
        public void NotifyNewMonitoredFile(string filePath)
        {
            if (string.IsNullOrWhiteSpace(filePath)) return;
            var dialog = _activeMaterialDialog;
            if (dialog != null && !dialog.IsDisposed && dialog.Visible)
            {
                dialog.AppendPendingFile(filePath);
            }
        }
            ...
            List<string> pendingBatchFiles = null)
        {
            ...
            try
            {
                ...
                _activeMaterialDialog = new MaterialSelectFormModern(
                    materials: materials,
                    fileName: fileName,
                ...
                var dialog = _activeMaterialDialog;
                if (pendingBatchFiles != null && pendingBatchFiles.Count > 0)
                {
                    dialog.SetPendingFiles(pendingBatchFiles);
                }
                ...
                try
                {
                    if (dialog.ShowDialog() == DialogResult.OK)
                    {
                        result = new MaterialSelectionResult
                        {
                            ...
                            SelectedTetBleed = dialog.SelectedTetBleed,
                            ...
                            IsForceRotationEnabled = dialog.IsForceRotationEnabled,
                            ...
                            TemporaryImpositionParameters = dialog.GetTemporaryImpositionParameters(),
                ...
                {
                    _activeMaterialDialog?.Dispose();
                    _activeMaterialDialog = null;
                }
            }
            catch (Exception ex)
            {
